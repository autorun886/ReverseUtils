...
# 处理结果，生成frida脚本
def generate_script(funclist, constlist, target_so_name):
    script_module = """
...
setImmediate(main);
    """
    hookscript = script_module.replace("$$$.so", target_so_name).replace("$$$const_addrs",str(constlist)).replace("$$$funcs",str(funclist))
    ...
    def run(self, arg):
        ...
        for sig in sig_list:
            ...
            while ea != None:
                ...
                idx = binary_data.find(sig["data"], ea + sig["size"])
                if idx != -1:
                    ea = idx
                else:
                    ea = None
            ...
            try:
                ...
                if len(Suspected_magic_num) >= 3:
                    if hex(func) in offsets:
                        ...
                        functionName = demangle_str(idc.get_func_name(func))
                        oneInfo["type"] = 2
        ...
        cur_so_path, cur_so_name = getSoPathAndName()
        myscript = generate_script(funclist, constlist, cur_so_name or "target.so")

        if cur_so_path and cur_so_name:
            script_name = cur_so_name.split(".")[0] + "_findhash_" + str(int(time.time())) + ".js"
            save_path = os.path.join(cur_so_path, script_name)
            with open(save_path, "w", encoding="utf-8") as F:
                F.write(myscript)
            print(f"生成对应的hook脚本如下：")
            print(f"frida -UF -l {save_path}")
        else:
            print("无法获取目标文件路径，跳过脚本保存。")
